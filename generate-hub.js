...
async function generateHub() {
    try {
        console.log('Memulai sinkronisasi proyek GitHub Pages...');
        const response = await fetch(`https://api.github.com/users/${USERNAME}/repos?per_page=100&sort=updated`);
        const repos = await response.json();

        // FILTER KETAT: Hanya repo dengan Pages aktif & bukan repo induk
        ...
        projects.forEach(repo => {
            ...
            projectCards += `
            <a href="${BASE_URL}${repo.name}/" target="_blank" class="glass p-6 rounded-3xl hover:bg-white/10 transition-all group border border-white/5 flex flex-col justify-between">
                <div>
                    <div class="flex justify-between items-start mb-6">
                        <div class="p-3 bg-blue-500/20 rounded-2xl text-blue-400 group-hover:scale-110 transition-transform">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
                            </svg>
                        </div>
                        <span class="text-[10px] font-bold uppercase tracking-widest text-slate-500 bg-slate-800/50 px-3 py-1 rounded-full">Active Pages</span>
                    </div>
        ...
        projects.forEach(repo => { sitemap += `  <url><loc>${BASE_URL}${repo.name}/</loc><priority>0.8</priority></url>\n`; });
        sitemap += `</urlset>`;
        fs.writeFileSync('sitemap.xml', sitemap);

        console.log(`✅ Sukses! ${projects.length} proyek GitHub Pages telah disinkronkan.`);
    } catch (error) {
